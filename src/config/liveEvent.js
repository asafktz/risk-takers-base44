// The site-wide floating mini player is NOT built here — it's a Flarea feature. Copy the
...
export const SHOWRUNNER_ORIGIN = 'https://flarea.ai';
...
export function showrunnerSlugFromUrl(url) {
  // New event URLs use flarea.ai; keep the permanent legacy redirector additive because old
  // episode records and links remain valid and must continue to resolve.
  const m = /(?:flarea\.ai|webinar-show\.vercel\.app)\/e\/([a-z0-9-]+)/i.exec(url || '');
  return m ? m[1] : null;
}
