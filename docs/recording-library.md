...
The public catalog contains 11 records and poster images only. `/api/replay?action=register` validates name/email, validates the request origin, and calls the service-only registration RPC. Successful persistence precedes the signed HTTP-only cookie. Existing registrations deduplicate by event and normalized email; no email or calendar message is sent to the registrant. New production registrations notify asaf@risktakers.show through Resend, including name, email, registration time and the replay link. Existing registrations are excluded. An atomic database claim and provider idempotency key prevent duplicate notifications; sent/failed state and provider ID are recorded. Notification failure is logged and never blocks viewing. Failed or interrupted notifications require operator review. Preview signups do not send alerts. The Admin registration view includes a separate Recording access source.

The private `ai-defense-recordings-20260924-v2` bucket stores HLS assets. Stream requests require an unexpired signed session and an existing registration. Each manifest signs the initialization object and every media segment for two hours. Anonymous bucket access is disabled. Session cookies last 30 days; deleting a registration revokes future manifest access. Already-issued segment URLs remain valid until their two-hour expiry.
...
The September 24 revision restores all five original presenter intro videos in the episode and at the start of each vendor presentation. Vendor presentations include the full retained panel discussion, preserving captions, portrait substitutions and technical/filler edits. The five standalone Q&A files remain unchanged. Public clip IDs and links remain stable.
